...
> **Version:** 1.1  
> **Date:** 2026-06-30  
> **Status:** Proof of Concept — Scaled to 10,000 Documents  
> **Author:** DC  
...
- Query embedding, dense retrieval, fusion, and reranking complete in **~5 seconds** for the current 10,000-document corpus (see Performance section below).
...
- **Current scale:** 10,000 documents for the POC
- **Scalable target:** 50,000+ documents
...
| 10,000 PubMed documents ingested | ✅ | Stored as section-aware chunks in Qdrant |
...
| Code committed locally | ✅ | Repository initialized and committed (local only) |
| Docker Compose orchestration | ✅ | Redis, Qdrant, API, worker, and UI services defined |

### Known Limitations
- Corpus size is intentionally small (10,000 docs) for rapid iteration and validation.
...
Measured after scaling the corpus from 5000 to 10,000 documents on the local CPU-only stack (WSL2, Docker Desktop, Celery concurrency=2).

| Metric | Value |
|--------|-------|
| Documents downloaded | 10,000 |
| Qdrant points (chunks) | 10,017 |
| Ingestion time | ~1814 seconds (~30.2 minutes) |
...
| Qdrant memory | 202.6 MiB / 4 GiB |
| Redis memory | 1.79 MB |

### Search Latency & Top Scores

| Query | Cache Miss Latency | Cache Hit Latency | Top Rerank Score |
|-------|--------------------|-------------------|------------------|
| cancer chemotherapy | 5.13 s | 7.5 ms | 0.9957 |
| diabetes treatment | 5.13 s | 4.0 ms | 0.9974 |
| Alzheimer's biomarkers | 5.13 s | 3.4 ms | 0.9994 |
| cardiovascular risk | 5.13 s | 3.6 ms | 0.9959 |
| gene therapy | 5.13 s | 3.6 ms | 0.8650 |

**Notes:**
- All five queries returned 5 results.
- Four of five queries produced top rerank scores above 0.95. The "gene therapy" query returned a still-strong top score of 0.86, reflecting a smaller number of directly relevant abstracts in the sampled 10,000-document subset.
...
| Real data processing | 10,000+ PubMed abstracts indexed | Qdrant collection count |
